...
import { 
  Search, Printer, Save, X, Beaker, Activity,
  PlusCircle, CheckCircle2, LayoutGrid, User, FlaskConical,
   Clock, Trash2, Droplets, Microscope, TestTube2, Stethoscope, BeakerIcon
} from "lucide-react";
...
const EXAMEN_CONFIG: Record<string, { color: string, icon: any, bg: string }> = {
  ...
  "Grupo Sanguíneo": { color: "text-red-800", bg: "bg-red-100", icon: BeakerIcon }, // Fallback HeartPulse o Beaker
...
export default function PanelControlMaster() {
  ...
  const handleSaveResults = async () => {
    ...
    } catch (e) { console.error("Error:", e); } finally { setIsSaving(false); }
  };
    ...
    <div className="flex h-screen bg-[#F1F5F9] overflow-hidden font-sans">
      ...
      {notification && (
        <div className="fixed top-8 left-1/2 -translate-x-1/2 z-[2000] bg-slate-900 text-white px-8 py-4 rounded-2xl shadow-2xl flex items-center gap-3 animate-in fade-in slide-in-from-top-4 duration-300">
          ...
          <span className="text-xs font-black uppercase tracking-[0.1em]">{notification}</span>
        </div>
      )}
      ...
      <aside className="w-85 bg-white border-r border-slate-200 flex flex-col shadow-xl z-20">
        <div className="p-8 pb-4">
          <div className="flex items-center gap-3 mb-8">
            ...
            <h2 className="text-sm font-black text-slate-800 uppercase tracking-tight">Pacientes del Día</h2>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3 custom-scrollbar">
          {pacientesHoy.map(p => (
            ...
            >
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-black ${selectedPacienteId === p.id ? "bg-white/20" : "bg-blue-50 text-blue-600"}`}>
                {p.nombre.charAt(0)}
              </div>
              <div className="text-left flex-1 min-w-0">
                <p className="font-black truncate text-xs uppercase tracking-tight">{p.nombre}</p>
                <p className={`text-[10px] font-bold opacity-70 ${selectedPacienteId === p.id ? "text-white" : "text-slate-400"}`}>{p.cedula}</p>
              </div>
              <span className={`text-[10px] font-black ${selectedPacienteId === p.id ? "text-white" : "text-blue-600"}`}>{p.completados}/{p.totalEx}</span>
            </button>
          ))}
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col overflow-hidden relative">
        {pacienteActivo ? (
          <div className="flex-1 flex flex-col p-10 space-y-8 overflow-hidden">
            <header className="flex justify-between items-center bg-white p-6 rounded-3xl shadow-sm border border-white">
              <div className="flex items-center gap-5">
                ...
                <div>
                  <h1 className="text-2xl font-black text-slate-900 uppercase tracking-tighter italic">{pacienteActivo?.nombre}</h1>
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{pacienteActivo?.cedula} • {new Date().toLocaleDateString()}</p>
                </div>
              </div>
            </header>
            ...
            <div className="flex-1 grid grid-cols-12 gap-8 overflow-hidden">
              ...
              <div className="col-span-8 flex flex-col min-h-0">
                <div className="flex items-center gap-2 mb-6 px-2 text-slate-500">
                  <LayoutGrid size={18} />
                  <h3 className="text-[10px] font-black uppercase tracking-[0.2em]">Estudios en el Laboratorio</h3>
                </div>
                ...
                <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 overflow-y-auto pr-4 pb-12 custom-scrollbar">
                  {examenesHoyPaciente.map((ex) => {
                    ...
                    return (
                      <div key={ex.id} className="bg-white p-1 rounded-[2.5rem] shadow-sm border border-slate-200 group relative flex flex-col">
                        ...
                        <div className="p-6 flex flex-col h-full">
                          <div className="flex items-center gap-4 mb-6">
                            <div className={`w-16 h-16 ${config.bg} ${config.color} rounded-[1.5rem] flex items-center justify-center shadow-inner`}>
                              <Icon size={32} />
                            </div>
                            <div className="flex-1">
                              <h4 className="font-black text-lg text-slate-800 uppercase tracking-tighter leading-none mb-2">{ex.tipo}</h4>
                              <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-[9px] font-black uppercase ${
                                ...
                                <Clock size={10} /> {ex.estado.replace('_', ' ')}
                              </div>
                            </div>
                          </div>
                ...
                <div className="space-y-3 overflow-y-auto flex-1 pr-2 custom-scrollbar">
                  {CATALOGO_EXAMENES.map(tipo => {
                    ...
                    return (
                      ...
                      >
                        <span className={`text-xs font-black uppercase tracking-tight ${yaExiste ? 'text-slate-400' : 'text-slate-600'}`}>
                          {tipo}
                        </span>
                        {!yaExiste && <PlusCircle size={18} className="text-blue-500 opacity-40 group-hover:opacity-100" />}
                        {yaExiste && <CheckCircle2 size={18} className="text-emerald-500" />}
                      </button>
                    );
                  })}
                </div>
          ...
          <div className="flex-1 flex flex-col items-center justify-center">
            ...
            <h2 className="text-[11px] font-black uppercase tracking-[0.5em] text-slate-300">Laboratorio Central</h2>
          </div>
        )}
      </main>
      ...
      {isEditing && activeExamen && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 md:p-8">
          <div className="absolute inset-0 bg-slate-900/70 backdrop-blur-md" onClick={() => !isSaving && setIsEditing(false)} />
          ...
          <div className="relative w-full max-w-6xl h-full bg-white rounded-[3rem] shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-300">
            ...
            <div className="p-8 bg-slate-50 border-b border-slate-200 flex justify-between items-center">
              <div className="flex items-center gap-6">
                <div className={`w-14 h-14 ${EXAMEN_CONFIG[activeExamen.tipo]?.bg || 'bg-blue-500'} ${EXAMEN_CONFIG[activeExamen.tipo]?.color || 'text-white'} rounded-2xl flex items-center justify-center shadow-lg shadow-black/5`}>
                  {React.createElement(EXAMEN_CONFIG[activeExamen.tipo]?.icon || FlaskConical, { size: 28 })}
                </div>
                <div>
                  <h2 className="text-2xl font-black uppercase italic tracking-tighter text-slate-900">{activeExamen.tipo}</h2>
                  <p className="text-[10px] font-bold text-blue-600 uppercase tracking-widest">{pacienteActivo?.nombre} • Resultado Clínico</p>
                </div>
              </div>
              <button onClick={() => setIsEditing(false)} className="w-12 h-12 flex items-center justify-center bg-white hover:bg-rose-50 hover:text-rose-500 rounded-full transition-all text-slate-400 border border-slate-100 shadow-sm">
                <X size={24} />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-10 custom-scrollbar bg-white">
              <div className="max-w-4xl mx-auto pb-20">
                ...
                <div className="mt-16 bg-slate-50 p-8 rounded-[2.5rem] border border-slate-200 shadow-inner">
                  <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 block text-center mb-6">Estado del Análisis</label>
            ...
            <div className="p-8 bg-white border-t border-slate-200 flex gap-4">
              <button onClick={() => setIsEditing(false)} className="px-10 py-5 bg-slate-100 text-slate-500 font-black rounded-2xl hover:bg-slate-200 transition-all uppercase text-[10px] tracking-widest">
                Descartar
              </button>
              <button 
              ...
              >
                {isSaving ? "Guardando..." : <><Save size={18} /> Confirmar y Guardar</>}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-slate-900/80 flex items-center justify-center z-[1100] p-4 backdrop-blur-sm">
          <div className="bg-white rounded-[3rem] w-full max-w-6xl h-[95vh] flex flex-col shadow-2xl overflow-hidden">
            <div className="flex justify-between items-center p-8 bg-white border-b">
              ...
              <button onClick={() => setShowPrintModal(false)} className="w-12 h-12 flex items-center justify-center hover:bg-slate-100 rounded-full transition-all"><X/></button>
            </div>
