...
export default function PacientesPage() {
  ...
  const loadPacientes = async () => {
    try {
      ...
      showNotification("error", "Error", "No se pudieron cargar los pacientes");
    }
  };
    ...
    try {
      if (editingPaciente && editingPaciente.id) {
        ...
        showNotification(
          "success",
          "Paciente Registrado",
          `${formData.nombre} ha sido añadido a la base de datos`
        );
      }
      loadPacientes();
      closeModal();
      ...
      showNotification("error", "Error", "No se pudo guardar el paciente");
    }
  };
      ...
      onConfirm: async () => {
        try {
          ...
          showNotification(
            "delete",
            "Registro Eliminado",
            `El paciente ${paciente?.nombre || ""} ha sido removido`
          );
          setPacientes((prev) => prev.filter((p) => p.id !== id));
        } catch (err) {
          console.error("Error deleting patient:", err);
          showNotification("error", "Error", "No se pudo eliminar el paciente");
        }
      },
    });
  };
              ...
              >
                ...
                <td className="px-6 py-4 text-right">
                  <div className="flex justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <button
                      onClick={() =>
                        paciente.id !== undefined && handleDelete(paciente.id)
                      }
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-bold text-slate-900 text-[11px] uppercase tracking-tight">
                    {paciente.nombre}
                  </p>
              ...
              <div className="flex gap-1">
                ...
                <button
                  onClick={() => openModal(paciente)}
                  className="p-2 text-slate-400 active:text-blue-500 transition-colors"
                >
                  <Edit2 size={16} />
                </button>
                <button
                  ...
                  className="p-2 text-slate-400 active:text-rose-500 transition-colors"
                >
                  <Trash2 size={16} />
                </button>
              </div>
            ...
            <div className="flex gap-2">
              <span className="px-2 py-0.5 bg-slate-50 text-[9px] font-bold text-slate-400 uppercase rounded border border-slate-100">
                {paciente.sexo || "N/A"}
              </span>
              <span className="px-2 py-0.5 bg-slate-50 text-[9px] font-bold text-slate-500 uppercase rounded border border-slate-100">
                {paciente.edad}
              </span>
            </div>
          </div>
          ...
          <div className="bg-white w-full max-w-md rounded-[2.5rem] p-8 shadow-2xl border border-slate-50">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="space-y-4">
                <div>
                  <label className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-2 block mx-1">
                    Nombre Completo
                  </label>
                  <input
                    type="text"
