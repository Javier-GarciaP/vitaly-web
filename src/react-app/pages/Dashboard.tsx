...
import { Users, TestTube, Clock, FileText, Activity, PieChart as PieIcon, ArrowUpRight, Calendar } from "lucide-react";
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 bg-white p-1.5 pr-4 rounded-2xl shadow-sm border border-slate-100 w-fit">
          ...
          <div>
            ...
            <p className="text-sm font-bold text-slate-700 leading-tight">Servidor Activo</p>
          </div>
        </div>
      </div>
          ...
          return (
            ...
            >
              <div className="flex justify-between items-start relative z-10">
                <div className={`p-3 rounded-2xl ${card.bgColor} ${card.iconColor} group-hover:scale-110 transition-transform`}>
                  <Icon size={24} />
                </div>
                <ArrowUpRight size={20} className="text-slate-200 group-hover:text-slate-400 transition-colors" />
              </div>
              ...
              {/* Decoración de fondo */}
              <div className={`absolute -right-4 -bottom-4 w-24 h-24 bg-gradient-to-br ${card.color} opacity-[0.03] rounded-full group-hover:scale-150 transition-transform duration-700`} />
            </div>
          );
        })}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white rounded-[2.5rem] shadow-sm border border-slate-100 p-8">
          <div className="flex items-center justify-between mb-8">
            <div>
              <h2 className="text-xl font-black text-slate-800 tracking-tight">Estadísticas de Análisis</h2>
              <p className="text-sm text-slate-400">Distribución por categoría de estudio</p>
            </div>
          ...
          <div className="h-[350px] w-full">
            {stats?.distribucion ? (
              ...
              <div className="h-full flex flex-col items-center justify-center gap-3">
                ...
                <p className="text-slate-400 font-medium text-sm">Analizando datos...</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="bg-slate-900 rounded-[2.5rem] p-8 shadow-xl shadow-blue-900/10 flex flex-col">
          <h2 className="text-xl font-black text-white tracking-tight mb-2">Acceso Rápido</h2>
          <p className="text-slate-500 text-sm mb-8">Operaciones frecuentes</p>
          
          <div className="space-y-3">
            {[
              { label: "Nuevo Paciente", sub: "Registro inicial", icon: Users, color: "bg-blue-500", href: "/pacientes" },
              { label: "Emitir Factura", sub: "Cobro de servicios", icon: FileText, color: "bg-emerald-500", href: "/facturas" },
              { label: "Cargar Resultados", sub: "Laboratorio clínico", icon: TestTube, color: "bg-violet-500", href: "/examenes" },
            ].map((link) => (
              <a
                key={link.label}
                href={link.href}
              ...
              >
                <div className="flex items-center gap-4">
                  <div className={`p-2.5 ${link.color} rounded-xl text-white shadow-lg`}>
                    <link.icon size={18} />
                  </div>
                  <div>
                    <h3 className="font-bold text-white text-sm">{link.label}</h3>
                    <p className="text-[10px] text-slate-500 uppercase font-black tracking-widest">{link.sub}</p>
                  </div>
                </div>
          ...
          <div className="mt-auto pt-8">
            <div className="bg-gradient-to-br from-blue-600 to-indigo-700 p-6 rounded-3xl relative overflow-hidden">
                <p className="text-white font-bold text-sm relative z-10">¿Necesitas ayuda?</p>
                <p className="text-blue-100 text-xs mt-1 relative z-10">Consulta el manual de usuario o contacta a soporte.</p>
                <Activity size={80} className="absolute -right-4 -bottom-4 text-white/10 rotate-12" />
            </div>
          </div>
        </div>
      </div>
