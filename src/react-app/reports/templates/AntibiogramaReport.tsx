...
const styles = StyleSheet.create({
  ...
  tableRow: { flexDirection: 'row', borderBottomWidth: 0.5, borderBottomColor: '#bfbfbf' },
  tableHeader: { backgroundColor: '#f9f9f9', borderBottomWidth: 1.5, borderBottomColor: '#6e2020' },
  tableColHeader: { fontSize: 8, fontWeight: 'bold', color: '#6e2020', padding: 4, textAlign: 'center' },
...
const AntibiogramaReport: React.FC<AntibiogramaReportProps> = ({ data, patient, qrImage }) => {
  return (
    <View style={styles.container}>
      <CommonHeader patient={patient} title="Bacteriología / Antibiograma" qrImage={qrImage}/>
      ...
      {data?.antibiograma_list && data.antibiograma_list.length > 0 && (
        <View style={styles.table}>
          <View style={[styles.tableRow, styles.tableHeader]}>
            <Text style={[styles.tableColHeader, styles.colAntibiotico]}>Antibiótico</Text>
            <Text style={[styles.tableColHeader, styles.colGermen]}>Sensib. A</Text>
            <Text style={[styles.tableColHeader, styles.colGermen]}>Sensib. B</Text>
          </View>
          {data.antibiograma_list.map((item, index) => (
            <View key={index} style={styles.tableRow}>
              <Text style={[styles.tableCol, styles.colAntibiotico]}>{item.nombre}</Text>
              <Text style={[styles.tableCol, styles.colGermen]}>{item.a || '-'}</Text>
              <Text style={[styles.tableCol, styles.colGermen]}>{item.b || '-'}</Text>
            </View>
          ))}
        </View>
      )}
